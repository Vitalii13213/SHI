...
def find_crystal_and_bomb_coordinates(image):
    ...
    for contour in gray_contours:
        if cv2.contourArea(contour) > 100:  # Фільтрація за розміром
            M = cv2.moments(contour)
            if M["m00"] != 0:
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
...
def get_selected_area():
    ...
    time.sleep(3)
    print("Початок роботи програми, натисніть старт")
    time.sleep(1)
...
def screenshot_and_click(x1, y1, width, height, model, end_time):
    sct = mss()
...
screenshot_and_click(x1, y1, width, height, model, end_time)
